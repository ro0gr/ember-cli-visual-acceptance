...
var reportToMarkdown = require('../utils').reportToMarkdown

function buildReport (params) {
  return new Promise(function (resolve, reject) {
    console.log('Sending to github')
    var jsonReport = JSON.parse(fs.readFileSync('visual-acceptance-report/report.json'))
    var markdownReport = reportToMarkdown(jsonReport)

    try {
      if (process.env.REPORT_MARKDOWN_PATH) {
        fs.writeFileSync(process.env.REPORT_MARKDOWN_PATH, markdownReport)
      }
    } catch (e) {
      console.log(e)
    }
